...
export async function generateStaticParams() {
  return getAllPoolIds().map((poolId) => ({ poolId }));
}
...
export async function generateMetadata({
  ...
  const pool = getPoolBySlug(params.poolId);
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `/${params.locale}/products/pools/${params.poolId}`,
      languages: {
        'en-US': `/en/products/pools/${params.poolId}`,
        'es-US': `/es/products/pools/${params.poolId}`,
        'pt-BR': `/pt/products/pools/${params.poolId}`,
      },
    },
    openGraph: {
      title,
      description,
      url: `${SITE_URL}/${params.locale}/products/pools/${params.poolId}`,
      ...
      <section className="container mx-auto px-4 py-8">
        <Breadcrumbs locale={locale}
          items={[
            ...
            { name: `${pool.modelCode} ${pool.name}`, href: `/products/pools/${poolId}` },
          ]}
        />
      </section>
                ...
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <div className={`grid gap-3 ${
                    ...
                    {pool.imagesIsoModels.map((src, i) => (
                      <ImageGalleryTrigger
                        key={i}
                        images={pool.imagesIsoModels || []}
                        alt={`${pool.name} 3D model render`}
                        initialIndex={i}
                      >
                        <div className="relative aspect-[4/3] rounded-xl overflow-hidden bg-pool-mist/50 border border-pool-deep/5">
                          <Image
                            src={src}
